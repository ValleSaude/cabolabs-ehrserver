...
class RestController {
    ...
    def login() {
        ...
        String username = params.username ?: json?.username
        String password = params.password ?: json?.password
        String organization_number = params.organization ?: json?.organization
...
//        def json = request.JSON
        ...
        if (plan_assoc) {
            if (plan_assoc.plan.repo_total_size_in_kb <= account.totalRepoSizeInKb) {
                commitLoggerService.log(request, null, false, null, session, params)
                renderError(message(code: 'rest.commit.error.cant_commit.insufficient_storage', args: [plan_assoc.plan.repo_total_size_in_kb, account.totalRepoSizeInKb]), '4507', 507)
        ...
        catch (XmlValidationException e) // xsd validation errors
        {
            ...
            log.error(e.message + " " + e.getClass().getSimpleName())
            // FIXME: the error might be more specific, see which errors we can have.
    ...
    def checkout(String ehrUid, String compositionUid, String format) {
        ...
        compositionUid = compositionUid?: json?.compositionUid
    ...
    def ehrCreate(String uid, String subjectUid, String format) {
        def json = request.JSON
        uid = uid ?: json.uid
        subjectUid = subjectUid ?: json.subjectUid
        format = format ?: json.format
    ...
    def queryCompositions() {
        ...
        // all are strings
        boolean retrieveData = request.JSON.retrieveData.toBoolean()
        // http://mrhaki.blogspot.com/2009/11/groovy-goodness-convert-string-to.html
